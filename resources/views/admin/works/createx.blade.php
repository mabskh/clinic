...
@section('title')
    @php echo $title = 'إضافة بيانات العمل' @endphp
@endsection
                                ...
                                <div
                                    ...
                                    <div class="d-flex align-items-baseline flex-wrap mr-5">
                                        ...
                                        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                                            ...
                                            <li class="breadcrumb-item">
                                                <a href="{{ route('admin.contacts.index') }}" class="text-muted">جهات
                                                    الاتصال</a>
                                            </li>
                                        </ul>
                                        <!--end::Breadcrumb-->
                                                ...
                                                <div class="card-header `">
                                                    <div class="card-title">
                                                        <h3 class="card-label" style="color: #d92815">{{ $title }}
                                                            <i class="mr-2"></i>
                                                            <small class="" style="color: #154dd9"> يتم اضافة بيانات
                                                            ...
                                                            <div class="form-group row">
                                                                ...
                                                                <div class="col-lg-11">
                                                                    <div class="row" id="work_selects">
                                                                        <select class="col-lg-3 mx-2 my-2 custom-select"
                                                                                id="main_work_cat">
                                                                            @if($patients && $patients -> count() > 0)
                                                                                @foreach($patients as $row)
                                                                                    <option
                                                                                        value="{{$row -> id }}">{{$row -> name}}</option>
                                                                                    <option
                                                                                        value="{{$row -> id }}1">{{$row -> name}}</option>
                                                                                @endforeach
                                                                            @endif
                                                                        </select>
...
@push('scripts')
    <script>
        function worksSelect() {
            ...
            $(this).nextAll().remove();
            $.get("", function (data) {});
            $(worksSelects).append('<select name="work_cat[]" class="col-lg-3 mx-2 my-2 custom-select"> <option value="aaqa">aqaa</option><option value="aaaa">aaaqa</option></select>');
            $(worksSelects).children().last().on('change', worksSelect);
        }

        $(document).ready(function () {
            $('#main_work_cat').on('change', function () {
                let worksSelects = $('#work_selects');
                $(worksSelects).find('select[name="work_cat[]"]').remove()
                $.get("", function (data) {
                    $(worksSelects).append('<select name="work_cat[]" class="col-lg-3 mx-2 my-2 custom-select"> <option value="aaa">aaa</option><option value="aaaa">aaaa</option></select>');
                    $(worksSelects).children().last().on('change', worksSelect);
                });
            });
        });
